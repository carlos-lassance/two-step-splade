...
namespace arg {
    ...
    struct Index: public Encoding {
        ...
        [[nodiscard]] auto index_filename2() const -> std::string const& { return m_index_2; }

      private:
        std::string m_index;
        std::string m_index_2;
    };
    ...
    struct Query: public Tokenizer {
        ...
        [[nodiscard]] auto query_file_2() -> std::optional<std::reference_wrapper<std::string const>>
        {
            if (m_query_file2) {
                return m_query_file2.value();
            }
            return std::nullopt;
        }
        ...
        [[nodiscard]] auto queries2() const -> std::vector<::pisa::Query>
        {
            std::vector<::pisa::Query> q;
            auto parse_query =
                resolve_query_parser(q, tokenizer(), m_term_lexicon, m_stop_words, m_stemmer);
            if (m_query_file2) {
                std::ifstream is(*m_query_file2);
                io::for_each_line(is, parse_query);
            } else {
                io::for_each_line(std::cin, parse_query);
            }
      ...
      private:
        ...
        std::optional<std::string> m_query_file2;
        int m_k = 0;
